...
for (const file of files) {
  ...
  if (!heroVideo) {
    ...
    const status = blockValue(heroVideo, 'status');
    const src = blockValue(heroVideo, 'src');
    const poster = blockValue(heroVideo, 'poster');
...
function blockValue(text, field) {
  return text.match(new RegExp('^\\s+' + field + ':\\s*["\\\']?([^\\n"\\\']+)["\\\']?\\s*
const files = walk(contentRoot).filter((file) => /\.(md|mdx)$/.test(file));
for (const file of files) {
  const text = readFileSync(file, 'utf8');
  if (value(text, 'status') !== 'published' || value(text, 'spatialExperienceVersion') !== '1') continue;
