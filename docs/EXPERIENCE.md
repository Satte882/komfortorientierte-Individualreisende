...
- Der Hero darf durch längere Copy **nicht automatisch wachsen**. Wenn H1 plus Basis-Overlay nicht sauber passen, wird der sichtbare Hero-Titel gekürzt oder typografisch angepasst; Lead/Meta werden nicht in den Hero hineingedrückt.
...
- Nach jedem Cinematic Hero folgt ein eigener **Post-Hero-Intro-Bereich** mit `--experience-intro-space`; der eigentliche Artikel-/Experience-Content beginnt erst danach mit `--experience-content-space`. Kein direktes „Hero endet → Content klebt an“.
